...
void write_output(std::string execution) {
    // 💡 Added std::ios::app flag to open the file in append mode.
    std::ofstream output_file("execution.txt", std::ios::app); 

    if (output_file.is_open()) {
        output_file << execution;
        output_file.close();  // Close the file when done
        
        // Changed the message to reflect the new behavior
